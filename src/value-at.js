...
function valueAt(x, index) {
  assert(
    !isUndefined(x),
    "You must pass an array and an index into the `valueAt` function!"
  )
  ...
  assert(
    ...
    "The index passed into the `valueAt` function must be a positive integer or a one-dimensional array of positive integers!"
  )
  ...
  if (isArray(index)) {
    ...
    assert(
      ...
      "The index passed into the `valueAt` function has too many dimensions!"
    )
  }
