...
function vectorize(fn) {
  ...
  return function temp() {
    let atLeastOneArgumentIsAnArray =
    ...
    if (atLeastOneArgumentIsAnArray) {
      let out = []
      let lengths = Object.keys(arguments)
...
if (!module.parent && typeof window === "undefined") {
  ...
  assert(
    isEqual(yTrue, yPred),
    "tens([0, 1, 2, 3]) should be [10, 10, 10, 10]!"
  )
  ...
  assert(
    isEqual(yTrue, yPred),
    "square([[[[1, 2, 3, 4]]]]) should be [[[[1, 4, 9, 16]]]]!"
  )
  ...
  assert(
    isEqual(yTrue, yPred),
    `foo(["a", "b", "c"]) should be ["afoo", "bfoo", "cfoo"]!`
  )
